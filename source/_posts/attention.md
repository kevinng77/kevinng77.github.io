...
<img src="/img/attention/image-20210530214554919.png" alt="image-20210530214554919" style="zoom:50%;" />

假设橙色点为我们已有数据集，蓝色曲线为$$y_i = 2\sin(x_i)+x_i^{0.8}$$ 是我们想要求的ground truth。

如果我们使用样本 $$y$$ 的均值作为预测值，那么就有了紫色虚线。显然我们的预测很差。

#### 没有参数的注意力池化 attention pooling

一个改进的方法是：我们根据输入x的位置，来给我们的输出 y 加上对应的权重。如下：
$$
...
$$
其中， $$\alpha(x,x_i)$$ 就是我们要给对应 $$y_i$$ 加上的权重。这时，我们考虑使用Gaussian kernel。
$$
...
<img src="/img/attention/image-20210530220922767.png" alt="image-20210530220922767" style="zoom:50%;" />

可以看到，在加上了没有参数的注意力池化之后，预测效果更好了。

<img src="/img/attention/image-20210530223554277.png" alt="image-20210530223554277" style="zoom:50%;" />
...
将我们的 $$(x-x_i)$$ 通过可训练矩阵 $$w$$ 投影之后再取 softmax，即换个角度来看Query和Key之间的关系。 更改后的方程如下：
$$
\begin{aligned}
f(x) &=\sum_{i=1}^{n} \alpha\left(x, x_{i}\right) y_{i} \\
...
<img src="/img/attention/image-20210530223416388.png" alt="image-20210530223416388" style="zoom:50%;" />

可以看到，还了一个观察角度。我们的注意力更集中了。

<img src="/img/attention/image-20210530223851183.png" alt="image-20210530223851183" style="zoom:50%;" />
...
$$
其中q, k和v分别代表 query，key和value。其中的 $$\alpha(q,k_i)$$ 经常是 softmax形式的函数：
$$
...
我们通常会将句子进行padding，填充以使每个batch中的句子长度相同，提升训练速度，因此需要将被填充的部分进行掩盖。
...
NMT 加入注意力之后表现更为优秀了，基于RNN的传统seq2seq模型有一个缺点便是信息瓶颈，即目标翻译句子的开始几个字将可以得到大部分的编码信息。随着翻译句子长度增加，后续的词解码时，hidden state中保留的编码信息就越来越少。那么注意力机制可以解决这个问题吗？
